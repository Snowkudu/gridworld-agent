...
def generate_dataset(
    ...
    for episode in range(episodes):
        ...
        episode_data = []
        while not done:
            action = OraclePolicy.select_action(
                env
            )  # will be random at one point, but for now we can use the oracle to generate the dataset
            actions.append(action)

            state = (
                env.state_vector()
            )  # Flatten the grid to a 1D array and convert to float32
            states.append(state)
...
def main():
    ...
    # init env
    env = GridWorld(
        10, 0.3, manhattan_shaped_reward, maxsteps=args.max_steps, seed=args.seed
    )
