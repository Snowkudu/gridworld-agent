...
def isSolvable(
    grid,
):  # Check if the grid is solvable using BFS from start to goal while avoiding obstacles.
    ...
    return (
        dist[agent_row][agent_col] != INF
    )  # If distance is INF, it means it's unreachable.


def _bfs_distance_from_goal(
    grid,
) -> np.ndarray:  # Returns an array with distances from the goal.
    tempgrid = grid.grid
    ...
    while queue:
        ...
        for itR, itC in actions:
            ...
            if (
                ...
                and dist_array[newRow][newCol] > dist_array[temp_row][temp_col] + 1
            ):
