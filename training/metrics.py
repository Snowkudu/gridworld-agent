...
def labels_from_subset(labels: torch.Tensor, subset: Subset) -> torch.Tensor:
...
def calculate_majority_baseline(
    ...
    *,
    ...
    def constant_accuracy(split_labels: torch.Tensor) -> float:
        ...
        return float((split_labels == majority_action).float().mean().item())

    return {
        "majority_action": majority_action,
        "training_action_counts": train_counts.tolist(),
        "train_accuracy": constant_accuracy(train_labels),
        "validation_accuracy": constant_accuracy(validation_labels),
